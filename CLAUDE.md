...
系统使用异步 **MessageBus**（两个 `asyncio.Queue`）解耦频道与 Agent：
...
- **`loop.py` - AgentLoop**：核心引擎。从消息总线接收消息，构建上下文，在工具调用循环中调用 LLM（最多 N 次迭代），执行工具，发送响应。关键方法：`_run_agent_loop()` 反复调用 LLM 直到没有更多工具调用或达到最大迭代次数。
...
内置工具：`read_file`、`write_file`、`edit_file`、`list_dir`、`exec`（Shell）、`web_search`、`web_fetch`、`message`、`spawn`、`cron`。MCP 工具从配置中动态加载。
...
- **`cron/`**：定时任务，支持 `every`（间隔）、`cron`（带时区）、`at`（一次性）三种调度方式。任务存储在 `~/.nano-alice/cron/jobs.json`。
- **`heartbeat/`**：Agent 周期性自检（每 30 分钟），使用 `workspace/HEARTBEAT.md` 作为提示词。
- **`session/`**：对话持久化，以 `"{channel}:{chat_id}"` 为键。

### 工作区 (`workspace/`)

引导文件加载到系统提示词：`AGENTS.md`（指令）、`SOUL.md`（人格）、`USER.md`（用户信息）、`TOOLS.md`、`IDENTITY.md`。记忆存储在 `memory/MEMORY.md` 和 `memory/HISTORY.md`。
