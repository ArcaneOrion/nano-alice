...
TaskPhase = Literal["planning", "executing", "blocked", "replanning", "completed"]
TaskStatus = Literal["active", "done", "failed", "cancelled"]
TaskStepStatus = Literal["pending", "in_progress", "done", "failed", "blocked"]
  ...
  <rule>若发现计划不成立，可进入 replanning，但必须先更新计划再继续执行。</rule>
    ...
    def render_task_state_xml(task_state: TaskState | None) -> str | None:
        ...
        for step in task_state.steps:
            ...
            plan_lines.append(
                f'    <step index="{step.index}" id="{escape(step.id)}" status="{step.status}">{title}</step>'
            )
